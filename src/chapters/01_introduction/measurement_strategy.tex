...
\begin{figure}[htbp]
  ...
  \includegraphics[width=\textwidth]{../resources/figures/chapters/01_introduction/ideal_positron_energy_spectra}
    \caption{
    Illustrative positron energy spectra for direct \(\pi\to e\nu\) decay and
    the sequential \(\pi\to\mu\to e\) chain in an ideal detector. The dashed line marks the analysis
    threshold separating the low-energy normalization region from the
    high-energy signal region.
    }
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{../resources/figures/chapters/01_introduction/timing_separation_prompt_view}
    \caption{
    Illustrative timing spectra for prompt \(\pi\to e\nu\) decay and the delayed
    \(\pi\to\mu\to e\) chain.
    }
...
\begin{figure}[htbp]
  ...
  \includegraphics[width=\textwidth]{../resources/figures/chapters/01_introduction/positron_tail_correction}
    \caption{
    Illustrative low-energy response tail for \(\pi\to e\nu\) positrons.
    Finite detector resolution and energy leakage move some signal events below
    \(E_{\mathrm{cut}}\), where they overlap the larger
    \(\pi\to\mu\to e\) spectrum.
    }
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccc}
    ...
    TRACK   & 50   & $[-300,700]$ & 3.4 & Broad normalization and accidental sample \\
